...
- CONFIG的内容：
  ...
  | WECOM_SMART_BOT_CHAT_ID | 企业微信智能机器人推送目标会话，单聊填用户 userid，群聊填群聊 chatid。支持多个目标用 `#` 分隔（如 `userA#userB`）逐条推送。需机器人已在该会话中收到过至少一条消息。单聊 userid 获取：机器人创建者为超级管理员时，运行 `python wecom_get_userids.py` 并让目标用户给机器人发一条消息即可打印其 userid |
...
- GitHub Actions 的 `schedule` cron 与 `CRON_HOURS` 变量均使用 **UTC（世界协调时）**，不是北京时间。
- 换算关系：**北京时间 = UTC + 8 小时**，配置时请将期望的北京时间减去 8。
- 常见换算示例：

  | 期望北京时间 | UTC 配置值 |
  |--------------|------------|
  | 8,10,12,14,16,18 | `0,2,4,6,8,10` |
  | 8,10,12,14,16,22 | `0,2,4,6,8,14` |
  | 10,14,18,22 | `2,6,10,14` |

- 常见误区：如果把北京时间 16 点误写成 UTC 16（配置里 `16`），实际会在**次日北京时间 0 点**执行；同理 UTC 22 对应**次日北京时间 6 点**。
- 修改配置后，建议在 Actions 中手动触发一次 `Random Cron` 工作流，让它同步更新 `run.yml` 中的 cron 表达式。
